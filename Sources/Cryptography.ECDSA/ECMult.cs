...
{
    internal class secp256k1_ecmult_context
    {
        //For accelerating the computation of a*P + b*G:
        public secp256k1_ge_storage[] pre_g;    //odd multiples of the generator
#if USE_ENDOMORPHISM
        public secp256k1_ge_storage[] pre_g_128; // odd multiples of 2^128*generator
#endif
    }

    internal class secp256k1_ecmult_gen_context
    {
        ...
        public secp256k1_ge_storage[][] prec; /* prec[j][i] = 16^j * i * G + U_i */
        public secp256k1_scalar blind;
        public secp256k1_gej initial;

        public void PrecInit()
        {
            prec = new secp256k1_ge_storage[64][];
            for (var i = 0; i < 64; i++)
            {
                prec[i] = new secp256k1_ge_storage[16];
                for (int j = 0; j < 16; j++)
                {
                    prec[i][j] = new secp256k1_ge_storage();
                }
            }
            blind = new secp256k1_scalar();
            initial = new secp256k1_gej();
        }
    }
    ...
    internal class ECMult
    {
        ...
        public static void secp256k1_ecmult_context_init(secp256k1_ecmult_context ctx)
        {
            ctx.pre_g = null;
#if USE_ENDOMORPHISM
            ctx.pre_g_128 = null;
        ...
        public static void secp256k1_ecmult_context_build(secp256k1_ecmult_context ctx, EventHandler<secp256k1_callback> cb)
        {
            if (ctx.pre_g != null)
                return;
            ...
            ctx.pre_g = new secp256k1_ge_storage[tblsize];
            for (int i = 0; i < tblsize; i++)
            {
                ctx.pre_g[i] = new secp256k1_ge_storage();
            }
            /* precompute the tables with odd multiples */
            secp256k1_ecmult_odd_multiples_table_storage_var(tblsize, ctx.pre_g, gj, cb);
